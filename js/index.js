...
function buildFeatured() {
  ...
  const artHtml = f.file
    ? `<img src="img/gallery/${f.file}" alt="${f.title}" class="art-canvas">`
...
function buildGallery() {
  ...
  GALLERY_PIECES.forEach((p, i) => {
    const artHtml = p.file
      ? `<img src="img/gallery/${p.file}" alt="${p.title}" class="art-canvas">`
...
function buildSpotlights() {
  ...
  SPOTLIGHTS.forEach((s, i) => {
    const artHtml = s.file
      ? `<img src="img/gallery/${s.file}" alt="${s.title}" class="art-canvas">`
...
function openLb(src) {
  ...
  if (src.file) {
    ...
    img.alt = ''; img.className = 'lightbox__canvas';
